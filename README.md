...
25. **Bluetooth speaker daemon** — turns the Pi into an A2DP sink advertised as `BT_SPEAKER_NAME` (default `RetroPieArcade`), using PipeWire/WirePlumber's native Bluetooth module (`libspa-0.2-bluetooth`) rather than `bluealsa` (not packaged for this Debian release) or a separate PulseAudio stack. Three systemd services back it: `bt-agent` (a headless `NoInputNoOutput` pairing agent from `bluez-tools` that auto-accepts incoming pairing with no PIN prompt), `bt-power-on` (powers the adapter on at boot only — it is deliberately *not* discoverable/pairable at boot; see the "Bluetooth Player" tool below for when it is), and `bt-nowplaying-daemon` (polls BlueZ's `MediaPlayer1`/`Device1` D-Bus objects for AVRCP now-playing metadata, auto-trusts newly paired devices, and executes play/pause/next/previous commands). The advertised name comes from `/etc/machine-info`'s `PRETTY_HOSTNAME` — BlueZ's `hostname` plugin overrides `main.conf`'s `Name=` with that value by default, so that's the file that actually controls what phones see. `loginctl enable-linger` keeps the `pi` user's PipeWire session (and so the A2DP sink) running even without an active console session.
26. **"Bluetooth Player" tool** — a car-infotainment-style now-playing screen (`~/scripts/bt-nowplaying.py`) for whatever's streaming from a phone connected to the speaker above: track title (marquee-scrolls if long), artist/album, progress bar, and connection status. Controller-navigable: left/right skips to the previous/next track, up/down adjusts the Pi's own output volume, confirm toggles play/pause, back exits — closing this tool does not stop playback, since the audio itself is handled entirely by the background daemon. The Pi only becomes discoverable/pairable to a *new* phone while this screen is open (it switches discoverable+pairable on at launch and back off on exit) — a phone that's already paired can reconnect and stream at any time regardless. To pair a phone for the first time: open this tool first, then pair from the phone's Bluetooth settings.
...
29. **"FTP settings" tool** — a one-button on/off switch (`~/scripts/ftp-settings.py`) for the ProFTPD server installed in step 7, which is enabled by default; this just wraps `systemctl enable/disable --now proftpd` with a status screen (shows the `ftp://` address to connect to when it's on).
...
- **Stream from a phone**: open *Bluetooth Player* in the RetroPie menu first — this is what makes the Pi discoverable/pairable, only while it's open — then pair the phone to `BT_SPEAKER_NAME` (`RetroPieArcade` by default) from the phone's Bluetooth settings (no PIN prompt on either side) and play something; the screen shows the track and lets you skip/pause from the controller. Once paired, the phone can reconnect and stream any time, even with this screen closed.
...
- **Debounce confirm/back button presses, not just directional ones.** A physical arcade button or joystick encoder can report more than one rapid press event for what's physically a single tap (contact bounce); for a one-shot action like toggling play/pause, an undebounced double-fire looks to the user like the button "did nothing" (it actually toggled twice). The Audio Player and Bluetooth Player tools track the last confirm/back timestamp and ignore a repeat within 250ms — directional movement is left alone since repeating on a held button is the desired behavior for menu navigation.
- **Bluetooth pairing is fully unattended by design** (`bt-agent -c NoInputNoOutput` auto-accepts "Just Works" pairing with no PIN on either side) — appropriate for a speaker/controller-pairing kiosk, but it does mean anything in Bluetooth range can pair without confirmation. Turn off `ENABLE_BT_SPEAKER` or stop `bt-agent.service` if that's not acceptable for your environment.
