...
# Wait up to ~10s for at least one display connector to report "connected"
# before launching the frontend. A monitor/TV that's slower to wake up or
# finish its own HDMI negotiation than the Pi is to boot - common with
# TVs and some cheap monitors coming out of standby, and confirmed live
# after a separate cable/hotplug-detection issue on the reference Pi -
# can otherwise leave the frontend already running against a GPU with no
# display attached at all: it starts and keeps running, but never
# acquires a working DRM display, and does NOT pick one up later even if
# the monitor comes alive seconds afterward (confirmed live: the only fix
# was killing and relaunching it after the display showed as connected).
# Proceeds anyway once the timeout is up either way, so a genuinely
# headless setup, or a connector this simple check doesn't recognize,
# can never be blocked forever - this is a best-effort mitigation for a
# common timing race, not a guarantee.
wait_for_display() {
    for i in \$(seq 1 20); do
        grep -q "^connected" /sys/class/drm/card*-*/status 2>/dev/null && return 0
        sleep 0.5
    done
    return 1
}
